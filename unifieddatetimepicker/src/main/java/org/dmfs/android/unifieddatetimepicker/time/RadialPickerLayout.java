...
/**
 * The primary layout to hold the circular picker, and the am/pm buttons. This view well measure itself to end up as a square. It also handles touches to be
...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener
{
    ...
    public interface OnValueSelectedListener
    {
        ...
        void onCycle(int pickerIndex, boolean forward);
    }
    ...
    private class CycleDetector
    {
        ...
        CycleDetector(boolean fineGrained)
        {
            mFineGrained = fineGrained;
        }


        boolean trackCycle(int pos)
        {
            pos = mFineGrained ? snapPrefer30s(pos) : snapOnly30s(pos, 0);
            ...
            forward = lastPos < 0 && pos >= 0 && pos < 90;
            backward = lastPos >= 0 && pos < 0 && pos > -90;
                           ...
                           PickerContext pickerContext)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            hoursTexts[i] = mPickerContext.use24Hours() ? String.format("%02d", hours_24[i]) : String.format("%d", hours[i]);
            innerHoursTexts[i] = String.format("%d", hours[i]);
            minutesTexts[i] = String.format("%02d", minutes[i]);
        }
        mHourRadialTextsView.initialize(res, hoursTexts, (mPickerContext.use24Hours() ? innerHoursTexts : null), true, true);
        mHourRadialTextsView.invalidate();
        mMinuteRadialTextsView.initialize(res, minutesTexts, null, true, false);
        ...
        mHourRadialSelectorView.initialize(context, true, mPickerContext.use24Hours(), true, hourDegrees, isHourInnerCircle(initialHoursOfDay));
        int minuteDegrees = initialMinutes * MINUTE_VALUE_TO_DEGREES_STEP_SIZE;
        mMinuteRadialSelectorView.initialize(context, true, false, false, minuteDegrees, false);
    ...
    private boolean isHourInnerCircle(int hourOfDay)
    {
        ...
        return mPickerContext.use24Hours() && (hourOfDay <= 12 && hourOfDay != 0);
    }
    ...
    /**
     * Split up the 360 degrees of the circle among the 60 selectable values. Assigns a larger selectable area to each of the 12 visible values, such that the
     * ratio of space apportioned to a visible value : space apportioned to a non-visible value will be 14 : 4. E.g. the output of 30 degrees should have a
     * higher range of input associated with it than the output of 24 degrees, because 30 degrees corresponds to a visible number on the clock circle (5 on the
    ...
    /**
     * Returns mapping of any input degrees (0 to 360) to one of 60 selectable output degrees, where the degrees corresponding to visible numbers (i.e. those
     * divisible by 30) will be weighted heavier than the degrees corresponding to non-visible numbers. See {@link #preparePrefer30sMap()} documentation for the
     * rationale and generation of the mapping.
     */
    private int snapPrefer30s(int degrees)
    {
        ...
        return mSnapPrefer30sMap[degrees] % 360;
    }


    /**
     * Returns mapping of any input degrees (0 to 360) to one of 12 visible output degrees (all multiples of 30), where the input will be "snapped" to the
     * closest visible degrees.
     *
     * @param degrees
     *         The input degrees
     * @param forceHigherOrLower
     *         The output may be forced to either the higher or lower step, or may be allowed to snap to whichever is closer. Use 1 to force strictly higher, -1
     *         to force strictly lower, and 0 to snap to the closer one.
     *
    ...
    /**
     * For the currently showing view (either hours or minutes), re-calculate the position for the selector, and redraw it at that position. The input degrees
     * will be snapped to a selectable value.
     *
     * @param degrees
     *         Degrees which should be selected.
     * @param isInnerCircle
     *         Whether the selection should be in the inner circle; will be ignored if there is no inner circle.
     * @param forceToVisibleValue
     *         Even if the currently-showing circle allows for fine-grained selection (i.e. minutes), force the selection to one of the visibly-showing values.
     * @param forceDrawDot
     *         The dot in the circle will generally only be shown when the selection is on non-visible values, but use this to force the dot to be shown.
     *
    ...
    private int reselectSelector(int degrees, boolean isInnerCircle, boolean forceToVisibleValue, boolean forceDrawDot)
    {
		...
		/*
         * if (currentShowing == HOUR_INDEX) { if (mIs24HourMode) { if (degrees == 0 && isInnerCircle) { degrees = 360; } else if (degrees == 360 &&
		 * !isInnerCircle) { degrees = 0; } } else if (degrees == 0) { degrees = 0; } } else if (degrees == 360 && currentShowing == MINUTE_INDEX) { degrees =
		 * 0; }
		 */
        int value = degrees / stepSize;
        if (currentShowing == HOUR_INDEX && (mPickerContext.use24Hours() && !isInnerCircle && degrees != 0
                || mPickerContext.use24Hours() && isInnerCircle && degrees == 0 || !mPickerContext.use24Hours() && (mCurrentHoursOfDay >= 12)))
        {
            value += 12;
        }
        return value;
    ...
    /**
     * Calculate the degrees within the circle that corresponds to the specified coordinates, if the coordinates are within the range that will trigger a
     * selection.
     *
     * @param pointX
     *         The x coordinate.
     ...
     *         The y coordinate.
     * @param forceLegal
     *         Force the selection to be legal, regardless of how far the coordinates are from the actual numbers.
     * @param isInnerCircle
     *         If the selection may be in the inner circle, pass in a size-1 boolean array here, inside which the value will be true if the selection is in the
     *         inner circle, and false if in the outer circle.
     *
    ...
    public void setCurrentItemShowing(int index, boolean animate)
    {
        ...
        {
            float hourAlpha = (index == HOUR_INDEX) ? 1f : 0;
            float minuteAlpha = (index == MINUTE_INDEX) ? 1f : 0;
        ...
        switch (event.getAction())
        {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mDownDegrees != -1)
                {
                    ...
                    // }
                    mCycleDetector = new CycleDetector(getCurrentItemShowing() == MINUTE_INDEX);
                    mCycleDetector.trackCycle(mDownDegrees);
                }
                return true;
            case MotionEvent.ACTION_MOVE:
                ...
                if (degrees != -1)
                {
                    ...
                    if ((!isInnerCircle[0] || !mPickerContext.use24Hours()) && mCycleDetector.trackCycle(degrees))
                    {
                        mListener.onCycle(getCurrentItemShowing(), mCycleDetector.forward);
                    }
                }
                return true;
            case MotionEvent.ACTION_UP:
				 ...
				 * mListener.onValueSelected(AMPM_INDEX, mIsTouchingAmOrPm, false); setValueForItem(AMPM_INDEX, isTouchingAmOrPm); } } mIsTouchingAmOrPm = -1;
				 * break; }
				 */
