 ...
 */
public abstract class DatePicker extends ListView implements OnScrollListener, DateTimePicker, MonthPickerAdapter.OnDateClickListener
{
    ...
    // highlighted time
    protected DateTime mSelectedDay = DateTime.nowAndHere();
    protected MonthPickerAdapter mAdapter;
    ...
    // private DatePickerController mPickerContext;
    ...
    public DatePicker(Context context, PickerContext pickerContext)
    {
        super(context);
    ...
    public DatePicker(Context context, AttributeSet attrs, PickerContext pickerContext)
    {
        super(context, attrs);
        init(context, pickerContext);
    }
    ...
    public void init(Context context, PickerContext pickerContext)
    {
    ...
    protected void refreshAdapter()
    {
        if (mAdapter == null)
        {
            mAdapter = createMonthAdapter(getContext(), mPickerContext);
    ...
    public abstract MonthPickerAdapter createMonthAdapter(Context context, PickerContext controller);
    ...
    /**
     * This moves to the specified time in the view. If the time is not already in range it will move the list so that the first of the month containing the
     ...
     */
    public boolean goTo(DateTime date, boolean animate, boolean setSelected, boolean forceScroll)
    {
        ...
        final int position = (date.getYear() - mPickerContext.minDateTime().getYear()) * MonthPickerAdapter.MONTHS_IN_YEAR + date.getMonth();
     ...
     */
    protected void setMonthDisplayed(DateTime date)
    {
    ...
    {
        // nothing to do
        goTo(mDateTime, false, true, true);
    }
     ...
     */
    private DateTime findAccessibilityFocus()
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            {
                final DateTime focus = ((MonthView) child).getAccessibilityFocus();
                if (focus != null)
                {
     ...
     */
    private boolean restoreAccessibilityFocus(DateTime day)
    {
        if (day == null)
        {
            return false;
    ...
    {
        final DateTime focusedDay = findAccessibilityFocus();
    ...
    private static String getMonthAndYearString(DateTime day)
    {
    ...
    public boolean performAccessibilityAction(int action, Bundle arguments)
    {
        ...
        int year = firstVisiblePosition / 12 + mPickerContext.minDateTime().getYear();
    ...
    public void onDateClick(DateTime dateTime)
    {
        ...
        {
            mDateTime = new DateTime(mDateTime.getCalendarMetrics(), mDateTime.getTimeZone(), dateTime.getYear(), dateTime.getMonth(), dateTime.getDayOfMonth(),
                    mDateTime.getHours(), mDateTime.getMinutes(), mDateTime.getSeconds());
        }
        if (mOnDateTimeSetListener != null)
        {
            mOnDateTimeSetListener.onDateTimeSet(dateTime);
        }

        if (!mDateTime.isAllDay())
        {
            mOnPickerStateChangeListener.onPickerStateChange(EditorComponent.HOURS);
        }
    }
}
