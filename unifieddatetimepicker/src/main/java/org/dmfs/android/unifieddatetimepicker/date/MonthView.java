...
public abstract class MonthView extends View
{
    ...
    /**
     * These params can be passed into the view to control how it appears. {@link #VIEW_PARAMS_WEEK} is the only required field, though the default values are
     * unlikely to fit most layouts correctly.
     */
    /**
    ...
    protected static final int DEFAULT_WEEK_START = Weekday.SU.ordinal();
    ...
    protected PickerContext mPickerContext;
    ...
    private int fetchColor(int resource)
    {
        TypedValue typedValue = new TypedValue();

        TypedArray a = getContext().obtainStyledAttributes(typedValue.data, new int[] { resource });
    ...
    public MonthView(Context context, AttributeSet attr)
    {
        ...
        mCalendar = DateTime.nowAndHere();

        mDayOfWeekTypeface = res.getString(R.string.day_of_week_label_typeface);
        mMonthTitleTypeface = res.getString(R.string.sans_serif);

        mDayTextColor = res.getColor(R.color.date_picker_text_normal);
        mTodayNumberColor = fetchColor(R.attr.colorAccent);
        mSelectedDayColor = fetchColor(R.attr.colorPrimary);
        ...
        mSelectedDayTextColor = res.getColor(android.R.color.white);
        ...
        mRowHeight = (res.getDimensionPixelOffset(R.dimen.date_picker_view_animator_height) - MONTH_DAY_LABEL_TEXT_SIZE - getMonthHeaderSize()) / MAX_NUM_ROWS;
    ...
    public void setDatePickerController(PickerContext controller)
    {
        mPickerContext = controller;
    }
    ...
    protected void initView()
    {
        ...
        mSelectedCirclePaint.setColor(mSelectedDayColor);
    ...
    /**
     * Sets all the parameters for displaying this week. The only required parameter is the week number. Other parameters have a default value and will only
     * update if a new value is included, except for focus month, which will always default to no focus month if no value is passed in. See
    ...
    public void setMonthParams(HashMap<String, Integer> params)
    {
        ...
        final DateTime today = DateTime.nowAndHere();
        mHasToday = false;
        mToday = -1;

        mCalendar = new DateTime(mYear, mMonth, 1);
        mDayOfWeekStart = mCalendar.getDayOfWeek();
        ...
        {
            mWeekStart = Calendar.getInstance().getFirstDayOfWeek() - 1;
        }

        mNumCells = mCalendar.getCalendarMetrics().getDaysPerPackedMonth(mYear, mMonth);
    ...
    private boolean sameDay(int day, DateTime today)
    {
        return mYear == today.getYear() && mMonth == today.getMonth() && day == today.getDayOfMonth();
    }
    ...
    {
        setMeasuredDimension(MeasureSpec.getSize(widthMeasureSpec), (mRowHeight) * mNumRows + getMonthHeaderSize() + MONTH_DAY_LABEL_TEXT_SIZE);
    }
    ...
    private String getMonthAndYearString()
    {
        ...
        long millis = mCalendar.getTimestamp();
        return DateUtils.formatDateRange(getContext(), mFormatter, millis, millis, flags, Time.getCurrentTimezone()).toString();
    }
    ...
    protected void drawMonthDayLabels(Canvas canvas)
    {
        ...
        for (int i = 0; i < mNumDays; i++)
        {
            ...
            float x = (2 * i + 1) * dayWidthHalf + mEdgePadding;

            canvas.drawText(shortWeekDays[calendarDay + 1], x, y, mMonthDayLabelPaint);
        }
    }
    ...
    protected void drawMonthNums(Canvas canvas)
    {
        ...
        for (int dayNumber = 1; dayNumber <= mNumCells; dayNumber++)
        {
            ...
            drawMonthDay(canvas, new DateTime(mYear, mMonth, dayNumber), x, y, startX, stopX, startY, stopY);
     ...
     */
    public abstract void drawMonthDay(Canvas canvas, DateTime date, int x, int y, int startX, int stopX, int startY, int stopY);
    ...
    {
        DateTime date = new DateTime(mYear, mMonth, day);
    ...
    /**
     * @return true if the specified year/month/day are within the range set by minDate and maxDate. If one or either have not been set, they are considered as
     * Integer.MIN_VALUE and Integer.MAX_VALUE.
     */
    protected boolean isOutOfRange(DateTime date)
    {
        DateTime minDate = mPickerContext.minDateTime();
        DateTime maxDate = mPickerContext.maxDateTime();
        return (minDate != null && minDate.after(date) || maxDate != null && maxDate.before(date));
    }
     ...
     */
    public DateTime getAccessibilityFocus()
    {
        final int day = mTouchHelper.getFocusedVirtualView();
        if (day >= 0)
        {
            return new DateTime(mYear, mMonth, day);
        }
        return null;
    }
     ...
     */
    public boolean restoreAccessibilityFocus(DateTime day)
    {
        if ((day.getYear() != mYear) || (day.getMonth() != mMonth) || (day.getDayOfMonth() > mNumCells))
        {
            return false;
        }
        mTouchHelper.setFocusedVirtualView(day.getDayOfMonth());
        return true;
    }
    ...
    protected class MonthViewTouchHelper extends ExploreByTouchHelper
    {
        ...
        {
            getAccessibilityNodeProvider(MonthView.this).performAction(virtualViewId, AccessibilityNodeInfoCompat.ACTION_ACCESSIBILITY_FOCUS, null);
        }
        ...
        public void clearFocusedVirtualView()
        {
            ...
            {
                getAccessibilityNodeProvider(MonthView.this).performAction(focusedVirtualView, AccessibilityNodeInfoCompat.ACTION_CLEAR_ACCESSIBILITY_FOCUS,
                        null);
            }
        }
        ...
        /**
         * Generates a description for a given time object. Since this description will be spoken, the components are ordered by descending specificity as DAY
         * MONTH YEAR.
         *
         * @param day
