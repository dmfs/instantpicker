...
public class RadialSelectorView extends View
{
    ...
    private static final int SELECTED_ALPHA = Utils.FULL_ALPHA;
    ...
    private final static float SMALL_CIRCLE_MULTIPLIER = 0.2f;
     ...
     *         Current context.
     * @param is24HourMode
     *         Whether the selector is in 24-hour mode, which will tell us whether the circle's center is moved up slightly to make room for the AM/PM circles.
     * @param hasInnerCircle
     *         Whether we have both an inner and an outer circle of numbers that may be selected. Should be true for 24-hour mode in the hours circle.
     * @param disappearsOut
     *         Whether the numbers' animation will have them disappearing out or disappearing in.
     * @param selectionDegrees
     *         The initial degrees to be selected.
     * @param isInnerCircle
     *         Whether the initial selection is in the inner or outer circle. Will be ignored when hasInnerCircle is false.
     */
    public void initialize(Context context, boolean is24HourMode, boolean hasInnerCircle, boolean disappearsOut, int selectionDegrees, boolean isInnerCircle)
    {
        ...
        mPaint.setColor(Utils.fetchPrimaryColor(context));
    ...
    /* package */ void setTheme(Context context, boolean themeDark)
    {
        ...
        {
            color = Utils.fetchPrimaryColor(context);
            mSelectionAlpha = SELECTED_ALPHA;
        }
     ...
     * @param forceDrawDot
     *         Whether to force the dot in the center of the selection circle to be drawn. If false, the dot will be drawn only when the degrees is not a
    ...
    private void prepareDrawValues()
    {
        if (mDrawValuesReady)
        {
            return;
        ...
        mSmallCircleRadius = (int) (mSelectionRadius * SMALL_CIRCLE_MULTIPLIER);
    ...
    public void onDraw(Canvas canvas)
    {
        ...
        canvas.drawCircle(mXCenter, mYCenter, mSmallCircleRadius, mPaint);

        if (mSelectionDegrees % 30 != 0)
        {
            // We're not on a direct tick
            mPaint.setAlpha(FULL_ALPHA);
            canvas.drawCircle(pointX, pointY, mSmallCircleRadius * 0.66f, mWhitePaint);
        }
        int lineLength = mLineLength + 1;
        ...
        mPaint.setAlpha(255);
        mPaint.setStrokeWidth(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2f, getResources().getDisplayMetrics()));
    ...
    public Path toPath()
    {
        ...
        mSelectionPath.addCircle(pointX, pointY, mSelectionRadius, Path.Direction.CW);
        return mSelectionPath;
    }
