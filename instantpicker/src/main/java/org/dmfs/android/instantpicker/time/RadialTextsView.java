...
public class RadialTextsView extends View
{
    ...
    private final static int[][] TEXT_POS_MAPPING = new int[][] {
            { 3, 0 }, { 4, 1 }, { 5, 2 }, { 6, 3 }, { 5, 4 }, { 4, 5 }, { 3, 6 }, { 2, 5 }, { 1, 4 },
            { 0, 3 }, { 1, 2 }, { 2, 1 } };
    ...
    public void onDraw(Canvas canvas)
    {
        ...
        if (mSelectionPath != null)
        {
            ...
            if (mHasInnerCircle)
            {
                drawTexts(canvas, mInnerTextSize, mTypefaceRegular, mInnerTexts, mInnerTextGridWidths, mInnerTextGridHeights, mWhitePaint);
            }

            canvas.clipRect(0, 0, getWidth(), getHeight(), Region.Op.XOR);
        }
        // Draw the texts in the pre-calculated positions.
        drawTexts(canvas, mTextSize, mTypefaceLight, mTexts, mTextGridWidths, mTextGridHeights, mPaint);
        if (mHasInnerCircle)
        {
            drawTexts(canvas, mInnerTextSize, mTypefaceRegular, mInnerTexts, mInnerTextGridWidths, mInnerTextGridHeights, mPaint);
        }
    ...
    /**
     * Using the trigonometric Unit Circle, calculate the positions that the text will need to be drawn at based on the specified circle radius. Place the
     * values in the textGridHeights and textGridWidths parameters.
     */
     ...
     */
    private void drawTexts(Canvas canvas, float textSize, Typeface typeface, String[] texts, float[] textGridWidths, float[] textGridHeights, Paint paint)
    {
        ...
        for (int i = 0; i < 12; ++i)
        {
            canvas.drawText(texts[i], textGridWidths[TEXT_POS_MAPPING[i][0]], textGridHeights[TEXT_POS_MAPPING[i][1]], paint);
        }
    }
